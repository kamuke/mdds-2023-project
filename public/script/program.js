...
filteredGenre.addEventListener('click', (event) => {
  ...
  activeGenreSelected.classList =
    'cursor-pointer inline-block px-2 py-1 bg-secondary-100 text-secondary-900 rounded-full font-semibold uppercase tracking-wide text-xs';

  //change selected genre styling
  event.target.classList =
    'cursor-pointer inline-block px-2 py-1 bg-secondary-900 text-secondary-100 rounded-full font-semibold uppercase tracking-wide text-xs';
  ...
  event.target.tagName === 'SPAN' &&
    renderMovieByGenre(event.target.textContent);
});
...
const renderMovie = (movies) => {
  ...
  filterMovieByTime.forEach((movie) => {
    ...
    startTimeText.innerHTML =
      movieTime.getHours() +
      '<span class="text-base pt-2">' +
      movieTime.getMinutes().toString().padStart(2, '0') +
      '</span>';
    startTimeDiv.classList =
      'animate-loadListing col-span-1 text-primary flex items-center justify-center';
    startTimeDiv.setAttribute(
      'style',
      'writing-mode: vertical-rl; text-orientation: upright;'
    );
    startTimeText.classList =
      'my-2 text-2xl font-semibold text-gray-100 border-l-4 border-primary';
    ...
    linkWrapper.classList =
      'movie-listing animate-loadListing lg:w-[500px] rounded-lg col-[span_11] lg:col-span-2 lg:col-start-3 lg:col-end-13 group cursor-pointer';
    divHoverElement.classList = 'group-hover:bg-secondary rounded-lg';
    container.classList =
      'bg-gray-900 rounded-lg shadow-xl hover:-translate-x-3 hover:-translate-y-3 flex flex-row md:flex-col';
    ...
    genre.classList =
      'inline-block px-2 py-1 bg-secondary-100 text-secondary-900 rounded-full font-semibold uppercase tracking-wide text-xs';
    ...
    length.innerHTML =
      ...
      movieTime.getMinutes().toString().padStart(2, '0') +
      '-' +
      movieEndTime.getHours() +
      ':' +
      movieEndTime.getMinutes().toString().padStart(2, '0');
    director.classList = 'whitespace-break-spaces text-gray-300 pt-2';
    director.innerHTML =
      '<span class="font-semibold">Director: </span><span>' +
      movie.director +
      '</span>';
    summary.classList = 'line-clamp-1 text-gray-300 pt-2';
    summary.innerHTML =
      '<span class="font-semibold">Summary: </span><span>' +
      movie.summary +
      '</span>';
